/**
 * Gets the URL of the website on current active page.
 * js will run consistently in the background.
 */
 ...
 var getCurrentTab = function() {
    chrome.tabs.query({
        ...
        //put url hostname and data into an var/obj
        var siteObj = {
            'title': hostname,
            'time': 0
        };
        // console.log(siteObj);
        //store siteObj var into local storage


        chrome.storage.local.set({'webTime': siteObj});
        chrome.storage.local.get(['webTime'], function(result) {
            console.log(result.webTime);
        });
    });
};
